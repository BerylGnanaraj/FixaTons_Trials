...
def attention_map(DATASET_NAME, STIMULUS_NAME):
    ...
    stimulus = cv2.cvtColor(get.stimulus(DATASET_NAME, STIMULUS_NAME),cv2.COLOR_RGB2BGR)
    saliency_map = get.saliency_map(DATASET_NAME, STIMULUS_NAME)
    heatmap_img = cv2.applyColorMap(saliency_map, cv2.COLORMAP_HOT)
    result = cv2.addWeighted(heatmap_img, 0.5, stimulus, 0.5, 0)
